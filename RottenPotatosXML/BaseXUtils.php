...
require ('BaseXClient.php');
    class BaseXUtils{
        ...
        static function getNews(){
            $query = self::$session->query('for $new in collection(\'../../../RottenPotatosXML/bbdd.xml\')//bbdd//news return $new');
            $xmlNew = $query->execute();
        ...
        static function getNewByKeyWords($keyWords){
            $xmlNew = self::$session->query('for $new in doc(\'../../../RottenPotatosXML/bbdd.xml\')//bbdd//news//new where $new//content[text() contains text { '.$keyWords.' } any] return $new');
            $json = json_encode($xmlNew);
            $arrayNews = json_decode($json,TRUE);
